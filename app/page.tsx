...
import OcrParcelScanner from "./components/OcrParcelScanner";
import BatchDatasetProcessor from "./components/BatchDatasetProcessor";
import {
  AlertCircle,
  AlertTriangle,
  ArrowRight,
...
const SAMPLE_PRESETS = [
  {
    label: "Ambattur Typo (Tamil Nadu)",
  ...
  {
    label: "Connaught Place (Delhi Circle)",
    text: "Block B Inner Circle, Near Rajiv Chowk Metro Gate 3, New Delhi - 110001",
    lang: "English",
  },
  {
    label: "Nariman Point & CST (Mumbai)",
    text: "Air India Building, Marine Drive Promenade, Nariman Point, Mumbai 400021",
    lang: "English",
  },
  {
    label: "PIN Mismatch Conflict (Bengaluru)",
    text: "Koramangala 5th block near Sony signal, PIN 600001",
    lang: "English",
  },
  {
    label: "Salt Lake Tech Hub (Kolkata)",
    text: "Sector V Bidhannagar, Near Karunamoyee Terminus, Kolkata 700064",
    lang: "English",
  },
];

const KNOWLEDGE_BASE_DATA = POST_OFFICES.map((po) => ({
  office: po.name,
  pin: po.pin_code,
  district: po.district,
  state: po.state,
  circle: po.circle,
  beats: po.delivery_beats_count,
  type: po.type,
  hub: po.hub_code,
  aliases: po.aliases,
}));

const SAMPLE_TRACKING_DATA = Object.entries(PARCELS).reduce((acc, [key, item]) => {
  acc[key] = {
    id: item.tracking_number,
    type: item.service_type,
    status: item.current_status,
    sender: `${item.sender_name}, ${item.sender_city}`,
    recipient: `${item.recipient_name}, ${item.recipient_address}`,
    originPIN: item.sender_pin,
    destPIN: item.recipient_pin,
    destOffice: item.assigned_office_name,
    beat: item.assigned_beat_name,
    eta: item.expected_delivery,
    rerouted: item.rerouted,
    rerouteReason: item.reroute_reason,
    checkpoints: (item.events || []).map((ev, idx) => ({
      time: ev.occurred_at,
...
export default function Home() {
  ...
  const [inputMode, setInputMode] = useState<"text" | "ocr" | "batch" | "speech">("text");
  const [analyzing, setAnalyzing] = useState(false);
  const [copied, setCopied] = useState(false);
  const [ocrUploaded, setOcrUploaded] = useState(false);

  // Master Datasets Explorer State
  const [datasetActiveTab, setDatasetActiveTab] = useState<"offices" | "localities" | "hubs" | "beats" | "parcels">("offices");
  const [datasetSearch, setDatasetSearch] = useState("");
  const [datasetCircleFilter, setDatasetCircleFilter] = useState("ALL");
  const [datasetCopied, setDatasetCopied] = useState<string | null>(null);
  ...
  // Lazily load tracking or route data when user visits those tabs
  useEffect(() => {
    if (activePage === "tracking" && !currentTracking) {
      handleSearchTracking(undefined, "SP102938475IN");
    }
    if (activePage === "routes" && !routeData) {
      fetchRouteData("600053");
    }
  }, [activePage, currentTracking, routeData]);
  ...
  const isStepAllowed = (step: AppPage, role: UserRole): boolean => {
    if (step === "auth" || step === "datasets") return true;
    if (role === "citizen") {
  ...
  if (!mounted) {
    return (
      <div className="layout-wrapper" style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div className="bg-mesh-grid" />
        <div className="liquid-glass" style={{ padding: "36px 48px", display: "flex", flexDirection: "column", alignItems: "center", gap: "16px", textAlign: "center" }}>
          <IndiaPostLogo size="md" variant="emblem" />
          <div style={{ fontWeight: 800, fontSize: "16px", color: "var(--text-main)", letterSpacing: "0.02em" }}>
            AI-POWERED DELIVERY POST OFFICE IDENTIFICATION SYSTEM
          </div>
          <div style={{ fontSize: "12px", color: "var(--accent-saffron)", fontFamily: "monospace", letterSpacing: "0.08em" }}>
            भारतीय डाक • DEPARTMENT OF POSTS • SECURE PORTAL INITIALIZING...
          </div>
        </div>
      </div>
    );
    ...
    <div className="layout-wrapper">
      ...
      <header className="app-header liquid-glass-header">
        ...
        <nav className="header-nav-strip">
          ...
          {/* Step 8: Master Postal Datasets & Directory */}
          <button
            className={`nav-tab-item ${activePage === "datasets" ? "active" : ""}`}
            onClick={() => handleNavigate("datasets")}
            title="Explore Integrated Master Postal Datasets, Post Offices, Localities & Hubs"
          >
            <Database size={15} />
            <span>8. 📚 Master Postal Datasets</span>
          </button>
        </nav>
      </header>
      ...
      <main className="app-body">
        ...
        {activePage === "verify" && (
          <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
            <div className="liquid-glass" style={{ padding: "24px" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: "14px" }}>
                ...
                <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
                  ...
                  <button
                    ...
                    style={{ fontSize: "12px" }}
                  >
                    <UploadCloud size={14} /> OCR Parcel Label
                  </button>
                  <button
                    className={`btn-secondary ${inputMode === "batch" ? "active" : ""}`}
                    onClick={() => setInputMode("batch")}
                    style={{
                      fontSize: "12px",
                      background: inputMode === "batch" ? "var(--accent-saffron)" : "rgba(245, 158, 11, 0.1)",
                      color: inputMode === "batch" ? "#000" : "var(--accent-saffron)",
                      fontWeight: 700,
                      borderColor: "rgba(245, 158, 11, 0.4)",
                    }}
                  >
                    <FileSpreadsheet size={14} /> ⚡ Huge Dataset Batch
                  </button>
                  <button
              ...
              {/* OCR Parcel Image Scanner */}
              {inputMode === "ocr" && (
                <OcrParcelScanner
                  onExtract={(extracted) => {
                    setAddressInput(extracted);
                    handleAnalyze(extracted);
                  }}
                  onSwitchToBatch={() => setInputMode("batch")}
                />
              )}

              {/* Huge Dataset Batch Processor */}
              {inputMode === "batch" && (
                <BatchDatasetProcessor
                  onInspectInStudio={(addr) => {
                    setInputMode("text");
            ...
            {inputMode !== "batch" && analysisResult && (
              <div style={{ display: "grid", gridTemplateColumns: "1.2fr 0.8fr", gap: "20px" }}>
        ...
        {/* ========================================================= */}
        {/* PAGE 8: 📚 MASTER POSTAL DATASETS & GIS DIRECTORY         */}
        {/* ========================================================= */}
        {activePage === "datasets" && (
          <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
            {/* Header Card with Dataset Metadata */}
            <div className="liquid-glass" style={{ padding: "28px" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: "16px" }}>
                <div>
                  <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                    <div style={{ padding: "8px", borderRadius: "10px", background: "rgba(235, 120, 23, 0.15)", color: "var(--accent-saffron)" }}>
                      <Database size={24} />
                    </div>
                    <div>
                      <h2 style={{ fontSize: "22px", fontWeight: 800, letterSpacing: "-0.01em" }}>
                        Integrated National Postal Datasets & Master GIS Directory
                      </h2>
                      <p style={{ fontSize: "13px", color: "var(--text-muted)", marginTop: "2px" }}>
                        Authoritative ground-truth datasets for India Post address parsing, post office identification, and hub routing.
                      </p>
                    </div>
                  </div>
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: "8px", marginTop: "14px" }}>
                    <span className="badge" style={{ background: "rgba(34, 197, 94, 0.15)", color: "var(--accent-emerald)", border: "1px solid rgba(34, 197, 94, 0.3)", padding: "4px 10px", borderRadius: "6px", fontSize: "11px", fontWeight: 700 }}>
                      ✓ DATASET VERIFIED: {POSTAL_DATASET_INFO.version}
                    </span>
                    <span className="badge" style={{ background: "rgba(255, 255, 255, 0.06)", border: "1px solid var(--glass-border-subtle)", padding: "4px 10px", borderRadius: "6px", fontSize: "11px", color: "var(--text-main)" }}>
                      🏛️ {POSTAL_DATASET_INFO.authority}
                    </span>
                    <span className="badge" style={{ background: "rgba(59, 130, 246, 0.15)", color: "#60a5fa", border: "1px solid rgba(59, 130, 246, 0.3)", padding: "4px 10px", borderRadius: "6px", fontSize: "11px" }}>
                      🌐 {POSTAL_DATASET_INFO.standard}
                    </span>
                    <span className="badge" style={{ background: "rgba(255, 255, 255, 0.06)", border: "1px solid var(--glass-border-subtle)", padding: "4px 10px", borderRadius: "6px", fontSize: "11px", color: "var(--text-muted)" }}>
                      🎯 Match Accuracy: {POSTAL_DATASET_INFO.accuracy_rate}
                    </span>
                  </div>
                </div>
                ...
                <div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
                  <button
                    ...
                    onClick={() => {
                      ...
                      a.href = url;
                      a.download = `IndiaPost_Master_Datasets_${POSTAL_DATASET_INFO.version}.json`;
                      a.click();
                      URL.revokeObjectURL(url);
                    }}
                  >
                    <Download size={14} /> Download JSON Dataset
                  </button>
                  ...
                  <button
                    ...
                    style={{ fontSize: "12px", display: "flex", alignItems: "center", gap: "6px", padding: "8px 14px" }}
                    onClick={() => {
                      const headers = "Name,PIN Code,District,State,Circle,Type,Hub Code,Delivery Beats,Latitude,Longitude\n";
                      const rows = POST_OFFICES.map(
                        (po) =>
                          `"${po.name}","${po.pin_code}","${po.district}","${po.state}","${po.circle}","${po.type}","${po.hub_code}",${po.delivery_beats_count},${po.latitude},${po.longitude}`
                      ).join("\n");
                      const blob = new Blob([headers + rows], { type: "text/csv" });
                      const url = URL.createObjectURL(blob);
                      const a = document.createElement("a");
                      a.href = url;
                      a.download = `IndiaPost_PostOffices_Directory_${POSTAL_DATASET_INFO.version}.csv`;
                      a.click();
                      URL.revokeObjectURL(url);
                    }}
                  >
                    <FileText size={14} /> Export Offices CSV
                  </button>

                  <a
                    href="/api/v1/datasets"
                    target="_blank"
                    rel="noreferrer"
                    className="btn-secondary"
                    style={{ fontSize: "12px", display: "flex", alignItems: "center", gap: "6px", padding: "8px 14px", textDecoration: "none" }}
                  >
                    <Server size={14} /> Live REST API
                  </a>

                  <button
                    className="btn-primary"
                    style={{ fontSize: "12px", display: "flex", alignItems: "center", gap: "6px", padding: "8px 14px", background: "var(--accent-saffron)", color: "#000", fontWeight: 800 }}
                    onClick={() => {
                      setActivePage("verify");
                      setInputMode("batch");
                    }}
                  >
                    <FileSpreadsheet size={14} /> ⚡ Batch Process Huge Dataset
                  </button>
                </div>
              </div>

              {/* 5 Dataset Metric Badges */}
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: "14px", marginTop: "22px" }}>
                <div
                  ...
                  style={{
                    ...
                    borderRadius: "12px",
                    cursor: "pointer",
                    border: datasetActiveTab === "offices" ? "2px solid var(--accent-saffron)" : "1px solid var(--glass-border-subtle)",
                    background: datasetActiveTab === "offices" ? "rgba(235, 120, 23, 0.08)" : undefined,
                  }}
                  onClick={() => setDatasetActiveTab("offices")}
                >
                  <div style={{ fontSize: "11px", color: "var(--text-muted)", fontWeight: 700 }}>1. POST OFFICES</div>
                  <div style={{ fontSize: "24px", fontWeight: 900, color: "var(--accent-saffron)", marginTop: "4px" }}>
                    {POST_OFFICES.length}
                  </div>
                  <div style={{ fontSize: "11px", color: "var(--text-muted)", marginTop: "2px" }}>Across 14 Circles</div>
                </div>
                ...
                <div
                  ...
                  style={{
                    ...
                    borderRadius: "12px",
                    cursor: "pointer",
                    border: datasetActiveTab === "localities" ? "2px solid var(--accent-emerald)" : "1px solid var(--glass-border-subtle)",
                    background: datasetActiveTab === "localities" ? "rgba(34, 197, 94, 0.08)" : undefined,
                  }}
                  onClick={() => setDatasetActiveTab("localities")}
                >
                  <div style={{ fontSize: "11px", color: "var(--text-muted)", fontWeight: 700 }}>2. LOCALITIES & CLUSTERS</div>
                  <div style={{ fontSize: "24px", fontWeight: 900, color: "var(--accent-emerald)", marginTop: "4px" }}>
                    {LOCALITIES.length}
                  </div>
                  <div style={{ fontSize: "11px", color: "var(--text-muted)", marginTop: "2px" }}>With PIN & Aliases</div>
                </div>
                ...
                <div
                  ...
                  style={{
                    ...
                    borderRadius: "12px",
                    cursor: "pointer",
                    border: datasetActiveTab === "hubs" ? "2px solid #60a5fa" : "1px solid var(--glass-border-subtle)",
                    background: datasetActiveTab === "hubs" ? "rgba(59, 130, 246, 0.08)" : undefined,
                  }}
                  onClick={() => setDatasetActiveTab("hubs")}
                >
                  <div style={{ fontSize: "11px", color: "var(--text-muted)", fontWeight: 700 }}>3. SORTING HUBS (NSH/ASH)</div>
                  <div style={{ fontSize: "24px", fontWeight: 900, color: "#60a5fa", marginTop: "4px" }}>
                    {HUBS.length}
                  </div>
                  <div style={{ fontSize: "11px", color: "var(--text-muted)", marginTop: "2px" }}>National & Air Hubs</div>
                </div>
                ...
                <div
                  ...
                  style={{
                    ...
                    borderRadius: "12px",
                    cursor: "pointer",
                    border: datasetActiveTab === "beats" ? "2px solid #c084fc" : "1px solid var(--glass-border-subtle)",
                    background: datasetActiveTab === "beats" ? "rgba(192, 132, 252, 0.08)" : undefined,
                  }}
                  onClick={() => setDatasetActiveTab("beats")}
                >
                  <div style={{ fontSize: "11px", color: "var(--text-muted)", fontWeight: 700 }}>4. DELIVERY BEATS</div>
                  <div style={{ fontSize: "24px", fontWeight: 900, color: "#c084fc", marginTop: "4px" }}>
                    {DELIVERY_BEATS.length}
                  </div>
                  <div style={{ fontSize: "11px", color: "var(--text-muted)", marginTop: "2px" }}>Carrier Route Coverage</div>
                </div>
                ...
                <div
                  ...
                  style={{
                    ...
                    borderRadius: "12px",
                    cursor: "pointer",
                    border: datasetActiveTab === "parcels" ? "2px solid #f43f5e" : "1px solid var(--glass-border-subtle)",
                    background: datasetActiveTab === "parcels" ? "rgba(244, 63, 94, 0.08)" : undefined,
                  }}
                  onClick={() => setDatasetActiveTab("parcels")}
                >
                  <div style={{ fontSize: "11px", color: "var(--text-muted)", fontWeight: 700 }}>5. SEED CONSIGNMENTS</div>
                  <div style={{ fontSize: "24px", fontWeight: 900, color: "#f43f5e", marginTop: "4px" }}>
                    {Object.keys(PARCELS).length}
                  </div>
                  <div style={{ fontSize: "11px", color: "var(--text-muted)", marginTop: "2px" }}>Multi-hop Event Logs</div>
                </div>
              </div>
            </div>
            ...
            <div className="liquid-glass" style={{ padding: "16px 20px" }}>
              <div style={{ display: "flex", flexWrap: "wrap", gap: "12px", alignItems: "center", justifyContent: "space-between" }}>
                ...
                <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
                  <button
                    className={`nav-tab-item ${datasetActiveTab === "offices" ? "active" : ""}`}
                    onClick={() => setDatasetActiveTab("offices")}
                    style={{ fontSize: "12px", padding: "6px 14px" }}
                  >
                    <Building size={14} /> Master Post Offices ({POST_OFFICES.length})
                  </button>
                  <button
                    className={`nav-tab-item ${datasetActiveTab === "localities" ? "active" : ""}`}
                    onClick={() => setDatasetActiveTab("localities")}
                    style={{ fontSize: "12px", padding: "6px 14px" }}
                  >
                    <MapPin size={14} /> Localities ({LOCALITIES.length})
                  </button>
                  <button
                    className={`nav-tab-item ${datasetActiveTab === "hubs" ? "active" : ""}`}
                    onClick={() => setDatasetActiveTab("hubs")}
                    style={{ fontSize: "12px", padding: "6px 14px" }}
                  >
                    <Truck size={14} /> Sorting Hubs ({HUBS.length})
                  </button>
                  <button
                    className={`nav-tab-item ${datasetActiveTab === "beats" ? "active" : ""}`}
                    onClick={() => setDatasetActiveTab("beats")}
                    style={{ fontSize: "12px", padding: "6px 14px" }}
                  >
                    <Navigation size={14} /> Delivery Beats ({DELIVERY_BEATS.length})
                  </button>
                  <button
                    className={`nav-tab-item ${datasetActiveTab === "parcels" ? "active" : ""}`}
                    onClick={() => setDatasetActiveTab("parcels")}
                    style={{ fontSize: "12px", padding: "6px 14px" }}
                  >
                    <Package size={14} /> Consignments ({Object.keys(PARCELS).length})
                  </button>
                </div>
                ...
                <div style={{ display: "flex", gap: "10px", alignItems: "center", flexWrap: "wrap" }}>
                  {/* Circle Dropdown */}
                  <select
                    value={datasetCircleFilter}
                    onChange={(e) => setDatasetCircleFilter(e.target.value)}
                    style={{
                      padding: "8px 12px",
                      borderRadius: "8px",
                      border: "1px solid var(--glass-border-subtle)",
                  ...
                  >
                    <option value="ALL">All Postal Circles</option>
                    <option value="Tamil Nadu">Tamil Nadu Circle</option>
                    <option value="Delhi">Delhi Circle</option>
                    <option value="Maharashtra">Maharashtra Circle</option>
                    <option value="Karnataka">Karnataka Circle</option>
                    <option value="West Bengal">West Bengal Circle</option>
                    <option value="Telangana">Telangana Circle</option>
                    <option value="Andhra Pradesh">Andhra Pradesh Circle</option>
                    <option value="Gujarat">Gujarat Circle</option>
                    <option value="Uttar Pradesh">Uttar Pradesh Circle</option>
                    <option value="Kerala">Kerala Circle</option>
                    <option value="Rajasthan">Rajasthan Circle</option>
                    <option value="Punjab">Punjab Circle</option>
            ...
            {/* TAB 1: MASTER POST OFFICES TABLE */}
            {datasetActiveTab === "offices" && (
              <div className="liquid-glass" style={{ padding: "20px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "14px" }}>
                  <h3 style={{ fontSize: "16px", fontWeight: 800, display: "flex", alignItems: "center", gap: "8px" }}>
                    <Building size={16} color="var(--accent-saffron)" /> Master Post Offices Directory
                  </h3>
                  <span style={{ fontSize: "12px", color: "var(--text-muted)", fontFamily: "JetBrains Mono" }}>
                    Showing {POST_OFFICES.filter((po) => {
                      const matchCircle = datasetCircleFilter === "ALL" || po.circle.toLowerCase().includes(datasetCircleFilter.toLowerCase());
                      const q = datasetSearch.toLowerCase().trim();
                      const matchQuery = !q || po.name.toLowerCase().includes(q) || po.pin_code.includes(q) || po.district.toLowerCase().includes(q) || po.aliases.some((a) => a.toLowerCase().includes(q));
                      return matchCircle && matchQuery;
                    }).length} of {POST_OFFICES.length} Offices
                  </span>
                </div>
                ...
                <div style={{ overflowX: "auto" }}>
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "12px" }}>
                    <thead>
                      <tr style={{ borderBottom: "1px solid var(--glass-border-subtle)", textAlign: "left", color: "var(--text-muted)", fontSize: "11px" }}>
                        <th style={{ padding: "10px 12px" }}>OFFICE NAME</th>
                        <th style={{ padding: "10px 12px" }}>PIN CODE</th>
                        <th style={{ padding: "10px 12px" }}>CIRCLE</th>
                        <th style={{ padding: "10px 12px" }}>TYPE</th>
                        <th style={{ padding: "10px 12px" }}>DISTRICT / STATE</th>
                        <th style={{ padding: "10px 12px" }}>SORTING HUB</th>
                        <th style={{ padding: "10px 12px" }}>BEATS</th>
                        <th style={{ padding: "10px 12px" }}>GIS COORDINATES</th>
                        <th style={{ padding: "10px 12px" }}>MAPPED ALIASES</th>
                        <th style={{ padding: "10px 12px" }}>ACTION</th>
                      </tr>
                    </thead>
                    <tbody>
                      {POST_OFFICES.filter((po) => {
                        const matchCircle = datasetCircleFilter === "ALL" || po.circle.toLowerCase().includes(datasetCircleFilter.toLowerCase());
                        const q = datasetSearch.toLowerCase().trim();
                        const matchQuery = !q || po.name.toLowerCase().includes(q) || po.pin_code.includes(q) || po.district.toLowerCase().includes(q) || po.aliases.some((a) => a.toLowerCase().includes(q));
                        return matchCircle && matchQuery;
                      }).map((po) => (
                        <tr key={po.id} style={{ borderBottom: "1px solid var(--glass-border-subtle)" }}>
                          <td style={{ padding: "12px", fontWeight: 700, color: "var(--text-main)" }}>
                            {po.name}
                          </td>
                          <td style={{ padding: "12px", fontFamily: "JetBrains Mono", color: "var(--accent-saffron)", fontWeight: 700 }}>
                            {po.pin_code}
                          </td>
                          <td style={{ padding: "12px", color: "var(--text-muted)" }}>{po.circle}</td>
                          <td style={{ padding: "12px" }}>
                            <span style={{ fontSize: "10px", padding: "2px 6px", borderRadius: "4px", background: po.type.includes("General") ? "rgba(235, 120, 23, 0.15)" : "rgba(255,255,255,0.06)", color: po.type.includes("General") ? "var(--accent-saffron)" : "inherit" }}>
                              {po.type}
                            </span>
                          </td>
                          <td style={{ padding: "12px" }}>{po.district}, {po.state}</td>
                          <td style={{ padding: "12px", fontFamily: "JetBrains Mono" }}>{po.hub_code}</td>
                          <td style={{ padding: "12px", fontFamily: "JetBrains Mono" }}>{po.delivery_beats_count} Beats</td>
                          <td style={{ padding: "12px", fontFamily: "JetBrains Mono", fontSize: "11px", color: "var(--text-muted)" }}>
                            {po.latitude.toFixed(4)}, {po.longitude.toFixed(4)}
                          </td>
                          <td style={{ padding: "12px", maxWidth: "200px" }}>
                            <div style={{ display: "flex", flexWrap: "wrap", gap: "4px" }}>
                              {po.aliases.slice(0, 3).map((a, i) => (
                                <span key={i} style={{ fontSize: "10px", padding: "1px 5px", borderRadius: "3px", background: "rgba(255,255,255,0.05)", color: "var(--text-muted)" }}>
                                  {a}
                                </span>
                              ))}
                              {po.aliases.length > 3 && (
                                <span style={{ fontSize: "10px", color: "var(--text-muted)" }}>+{po.aliases.length - 3}</span>
                              )}
                            </div>
                          </td>
                          <td style={{ padding: "12px" }}>
                            <button
                              ...
                              onClick={() => {
                                setAddressInput(`${po.name}, ${po.aliases[0] || po.district}, PIN ${po.pin_code}`);
                                setActivePage("verify");
                                setTimeout(() => {
                                  handleAnalyze(`${po.name}, ${po.aliases[0] || po.district}, PIN ${po.pin_code}`);
                                }, 300);
                              }}
                              title="Test address verification for this post office"
                            >
                              Verify Address →
                            </button>
                          </td>
            ...
            {/* TAB 2: MASTER LOCALITIES TABLE */}
            {datasetActiveTab === "localities" && (
              <div className="liquid-glass" style={{ padding: "20px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "14px" }}>
                  <h3 style={{ fontSize: "16px", fontWeight: 800, display: "flex", alignItems: "center", gap: "8px" }}>
                    <MapPin size={16} color="var(--accent-emerald)" /> Mapped Localities & Sub-City Clusters
                  </h3>
                  <span style={{ fontSize: "12px", color: "var(--text-muted)", fontFamily: "JetBrains Mono" }}>
                    Showing {LOCALITIES.filter((loc) => {
                      const matchCircle = datasetCircleFilter === "ALL" || loc.circle.toLowerCase().includes(datasetCircleFilter.toLowerCase());
                      const q = datasetSearch.toLowerCase().trim();
                      const matchQuery = !q || loc.name.toLowerCase().includes(q) || loc.pin_code.includes(q) || loc.district.toLowerCase().includes(q) || loc.aliases.some((a) => a.toLowerCase().includes(q));
                      return matchCircle && matchQuery;
                    }).length} of {LOCALITIES.length} Localities
                  </span>
                </div>
                ...
                <div style={{ overflowX: "auto" }}>
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "12px" }}>
                    <thead>
                      <tr style={{ borderBottom: "1px solid var(--glass-border-subtle)", textAlign: "left", color: "var(--text-muted)", fontSize: "11px" }}>
                        <th style={{ padding: "10px 12px" }}>LOCALITY NAME</th>
                        <th style={{ padding: "10px 12px" }}>PIN CODE</th>
                        <th style={{ padding: "10px 12px" }}>MAPPED POST OFFICE ID</th>
                        <th style={{ padding: "10px 12px" }}>DISTRICT</th>
                        <th style={{ padding: "10px 12px" }}>STATE & CIRCLE</th>
                        <th style={{ padding: "10px 12px" }}>SYNONYMS & LANDMARK ALIASES</th>
                        <th style={{ padding: "10px 12px" }}>ACTION</th>
                      </tr>
                    </thead>
                    <tbody>
                      {LOCALITIES.filter((loc) => {
                        ...
                        <tr key={loc.id} style={{ borderBottom: "1px solid var(--glass-border-subtle)" }}>
                          <td style={{ padding: "12px", fontWeight: 700, color: "var(--text-main)" }}>
                            {loc.name}
                          </td>
                          <td style={{ padding: "12px", fontFamily: "JetBrains Mono", color: "var(--accent-emerald)", fontWeight: 700 }}>
                            {loc.pin_code}
                          </td>
                          <td style={{ padding: "12px", fontFamily: "JetBrains Mono", color: "var(--text-muted)" }}>
                            {loc.post_office_id}
                          </td>
                          <td style={{ padding: "12px" }}>{loc.district}</td>
                          <td style={{ padding: "12px", color: "var(--text-muted)" }}>{loc.state} • {loc.circle}</td>
                          <td style={{ padding: "12px" }}>
                            <div style={{ display: "flex", flexWrap: "wrap", gap: "4px" }}>
                              {loc.aliases.map((a, i) => (
                                <span key={i} style={{ fontSize: "10px", padding: "1px 5px", borderRadius: "3px", background: "rgba(255,255,255,0.05)", color: "var(--text-muted)" }}>
                          ...
                          <td style={{ padding: "12px" }}>
                            <button
                              ...
                              style={{ fontSize: "11px", padding: "4px 8px", whiteSpace: "nowrap" }}
                              onClick={() => {
                                setAddressInput(`${loc.name}, near main bazaar, ${loc.district} PIN ${loc.pin_code}`);
                                setActivePage("verify");
                                setTimeout(() => {
                                  handleAnalyze(`${loc.name}, near main bazaar, ${loc.district} PIN ${loc.pin_code}`);
                                }, 300);
                              }}
                            >
                              Test In Studio →
            ...
            {/* TAB 3: MASTER SORTING HUBS TABLE */}
            {datasetActiveTab === "hubs" && (
              <div className="liquid-glass" style={{ padding: "20px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "14px" }}>
                  <h3 style={{ fontSize: "16px", fontWeight: 800, display: "flex", alignItems: "center", gap: "8px" }}>
                    <Truck size={16} color="#60a5fa" /> Authoritative Sorting Hubs & Air Cargo Directory
                  </h3>
                  <span style={{ fontSize: "12px", color: "var(--text-muted)", fontFamily: "JetBrains Mono" }}>
                    {HUBS.length} Sorting Hubs Operational
                  </span>
                </div>
                ...
                <div style={{ overflowX: "auto" }}>
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "12px" }}>
                    <thead>
                      <tr style={{ borderBottom: "1px solid var(--glass-border-subtle)", textAlign: "left", color: "var(--text-muted)", fontSize: "11px" }}>
                        <th style={{ padding: "10px 12px" }}>HUB CODE</th>
                        <th style={{ padding: "10px 12px" }}>HUB NAME</th>
                        <th style={{ padding: "10px 12px" }}>TYPE</th>
                        <th style={{ padding: "10px 12px" }}>CIRCLE / REGION</th>
                        <th style={{ padding: "10px 12px" }}>DAILY CAPACITY</th>
                        <th style={{ padding: "10px 12px" }}>EXPRESS AIR LANE</th>
                        <th style={{ padding: "10px 12px" }}>GPS COORDINATES</th>
                      </tr>
                    </thead>
                    <tbody>
                      {HUBS.map((hub) => (
                        <tr key={hub.id} style={{ borderBottom: "1px solid var(--glass-border-subtle)" }}>
                          <td style={{ padding: "12px", fontFamily: "JetBrains Mono", fontWeight: 800, color: "#60a5fa" }}>
                            {hub.code}
                          </td>
                          <td style={{ padding: "12px", fontWeight: 700 }}>{hub.name}</td>
                          <td style={{ padding: "12px" }}>
                            <span style={{ fontSize: "10px", padding: "2px 6px", borderRadius: "4px", background: hub.hub_type === "ASH" ? "rgba(192, 132, 252, 0.15)" : "rgba(59, 130, 246, 0.15)", color: hub.hub_type === "ASH" ? "#c084fc" : "#60a5fa" }}>
                              {hub.hub_type === "ASH" ? "✈️ AIR HUB" : hub.hub_type === "NSH" ? "🚚 NATIONAL HUB" : "🔄 INTRA-CIRCLE"}
                            </span>
                          </td>
                          <td style={{ padding: "12px", color: "var(--text-muted)" }}>{hub.circle} ({hub.district})</td>
                          <td style={{ padding: "12px", fontFamily: "JetBrains Mono" }}>
                            {hub.daily_capacity_bags.toLocaleString("en-IN")} Bags/Day
                          </td>
                          <td style={{ padding: "12px" }}>
                            {hub.express_lane ? (
                              <span style={{ color: "var(--accent-emerald)", fontWeight: 700 }}>✓ ACTIVE</span>
                            ) : (
                              <span style={{ color: "var(--text-muted)" }}>Standard Surface</span>
                            )}
                          </td>
                          <td style={{ padding: "12px", fontFamily: "JetBrains Mono", fontSize: "11px", color: "var(--text-muted)" }}>
            ...
            {/* TAB 4: CARRIER DELIVERY BEATS TABLE */}
            {datasetActiveTab === "beats" && (
              <div className="liquid-glass" style={{ padding: "20px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "14px" }}>
                  <h3 style={{ fontSize: "16px", fontWeight: 800, display: "flex", alignItems: "center", gap: "8px" }}>
                    <Navigation size={16} color="#c084fc" /> Carrier Delivery Beats & Postman Allocation
                  </h3>
                  <span style={{ fontSize: "12px", color: "var(--text-muted)", fontFamily: "JetBrains Mono" }}>
                    {DELIVERY_BEATS.length} Registered Postal Beats
                  </span>
                </div>
                ...
                <div style={{ overflowX: "auto" }}>
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "12px" }}>
                    <thead>
                      <tr style={{ borderBottom: "1px solid var(--glass-border-subtle)", textAlign: "left", color: "var(--text-muted)", fontSize: "11px" }}>
                        <th style={{ padding: "10px 12px" }}>POST OFFICE ID</th>
                        <th style={{ padding: "10px 12px" }}>BEAT NUMBER</th>
                        <th style={{ padding: "10px 12px" }}>BEAT NAME</th>
                        <th style={{ padding: "10px 12px" }}>ASSIGNED POSTMAN</th>
                        <th style={{ padding: "10px 12px" }}>AREAS & ROADS COVERED</th>
                        <th style={{ padding: "10px 12px" }}>MOBILE CONTACT</th>
                      </tr>
                    </thead>
                    <tbody>
                      {DELIVERY_BEATS.map((beat) => (
                        <tr key={beat.id} style={{ borderBottom: "1px solid var(--glass-border-subtle)" }}>
                          <td style={{ padding: "12px", fontFamily: "JetBrains Mono", color: "var(--text-muted)" }}>
                            {beat.post_office_id}
                          </td>
                          <td style={{ padding: "12px", fontFamily: "JetBrains Mono", fontWeight: 700, color: "var(--accent-saffron)" }}>
                            #{beat.beat_number}
                          </td>
                          <td style={{ padding: "12px", fontWeight: 700 }}>{beat.beat_name}</td>
                          <td style={{ padding: "12px", color: "var(--text-main)" }}>{beat.postman_name}</td>
                          <td style={{ padding: "12px", color: "var(--text-muted)" }}>{beat.areas_covered}</td>
                          <td style={{ padding: "12px", fontFamily: "JetBrains Mono", fontSize: "11px" }}>
                            {beat.mobile_contact || "Terminal Unit"}
                          </td>
                        </tr>
                      ))}
                    </tbody>
            ...
            {/* TAB 5: SEED CONSIGNMENTS TABLE */}
            {datasetActiveTab === "parcels" && (
              <div className="liquid-glass" style={{ padding: "20px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "14px" }}>
                  <h3 style={{ fontSize: "16px", fontWeight: 800, display: "flex", alignItems: "center", gap: "8px" }}>
                    <Package size={16} color="#f43f5e" /> Pre-Seeded Consignments & Tracking Master Records
                  </h3>
                  <span style={{ fontSize: "12px", color: "var(--text-muted)", fontFamily: "JetBrains Mono" }}>
                    {Object.keys(PARCELS).length} Consignments Available for Live Tracking
                  </span>
                </div>
                  ...
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "12px" }}>
                    <thead>
                      <tr style={{ borderBottom: "1px solid var(--glass-border-subtle)", textAlign: "left", color: "var(--text-muted)", fontSize: "11px" }}>
                        ...
                        <th style={{ padding: "10px 12px" }}>RECIPIENT & ADDRESS</th>
                        <th style={{ padding: "10px 12px" }}>ASSIGNED DELIVERY PO</th>
                        <th style={{ padding: "10px 12px" }}>ASSIGNED BEAT</th>
                    ...
                    <tbody>
                      {Object.values(PARCELS).map((parcel) => (
                        <tr key={parcel.id} style={{ borderBottom: "1px solid var(--glass-border-subtle)" }}>
                          <td style={{ padding: "12px", fontFamily: "JetBrains Mono", fontWeight: 800, color: "var(--accent-saffron)" }}>
                          ...
                          <td style={{ padding: "12px" }}>{parcel.sender_name} ({parcel.sender_city} - {parcel.sender_pin})</td>
                          <td style={{ padding: "12px" }}>{parcel.recipient_name}, {parcel.recipient_address} (PIN: {parcel.recipient_pin})</td>
                          <td style={{ padding: "12px", fontWeight: 700 }}>{parcel.assigned_office_name}</td>
                          <td style={{ padding: "12px", fontFamily: "JetBrains Mono", fontSize: "11px" }}>{parcel.assigned_beat_name}</td>
                          <td style={{ padding: "12px" }}>
                            <span style={{
                              fontSize: "11px",
                              padding: "2px 8px",
                              borderRadius: "12px",
                              background: parcel.current_status.includes("Delivered") ? "rgba(34, 197, 94, 0.15)" : "rgba(235, 120, 23, 0.15)",
                              color: parcel.current_status.includes("Delivered") ? "var(--accent-emerald)" : "var(--accent-saffron)",
                              fontWeight: 700,
                            }}>
                          ...
                          <td style={{ padding: "12px" }}>
                            <button
                              ...
                              onClick={() => {
                                setTrackingId(parcel.tracking_number);
                                setActivePage("tracking");
                                setTimeout(() => {
                                  handleSearchTracking(undefined, parcel.tracking_number);
                                }, 200);
                              }}
                            >
                              Track Live →
